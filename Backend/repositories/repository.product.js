...
exports.getProducts = async function (req,res){
    try{
        ...
        res.status(200).json({ success: true, message: "Successfully get products", data: result});
    }
    catch (error) {
        console.log(error);
...
exports.getProductBySupplier = async function (req, res) {
    try {
        ...
        res.status(200).json({ success: true, message: "Successfully retrieved products", data: products });
    } catch (error) {
        console.log(error);
        res.status(500).send("Internal Server Error");
    }
